...
To win a battle, it is important for competitive players to build teams with a wide range of types and moves rather than simply those with the highest base statistics, since nuances in moves and Pokemon typing add significant complexity to the battle system. For instance, moves are labelled by types and may deal extra ("super effective") or reduced damage based on the defending Pokemon's type(s) (see Figure 1 below). Moves are further classified as "physical" or "special". Moves have base power statistics which are amplified if the move type matches the user (called same-type attack bonus, or STAB). Physical moves use the attacking Pokemon's attack against the defender's HP and defence to calculate damage, while special moves use special attack against HP and special defence. Finally, non-damaging moves have a variety of effects like temporarily changing a Pokemon's stats, healing, causing weather or status conditions (e.g. hail, rain, poison, burn) which can affect Pokemon on the field, and creating hazards that damage or weaken Pokemon upon switching in.
...
- *Linear Models (LMs)*: The fitting of a line to data that is assumed to follow $Y|X \sim \mathcal{N}(\mu, \sigma^2)$. This was performed on `log(usage)`, making it equivalent to a generalized linear model with a log-normal likelihood.

- *Generalized Linear Models (GLMs)* (`MASS`): An extended form of a linear regression that allows $Y|X$ to follow any distribution. The quasi-poisson and negative binomial distributions are considered.

- *Generalized Additive Models (GAMs)* (`mgcv`): A GLM or LM with an additional non-linear smooth function for some coefficients that allow for fitting to non-linear trends.

- *Random Forests* (`randomForest`): The averaged predictions of many decision trees, a type of machine learning model that iteratively splits features based on thresholds into "purer", less varied subgroups to make predictions.
...
The decision tree models were trained on the same 80%/20% train/test split. The random forest consisted of 50 trees, while XGBoost was trained on 100 rounds of max depth 3, with $\eta = \gamma = 0.5$ and 10-fold cross-validation. To compare the models, training and testing set, root mean squared error (RMSE) is used, defined as $\text{RMSE} = \sqrt{\frac{1}{n}\sum_{i = 1}^n(y_i - t_i)^2}$, where $y_i$ and $t_i$ are the model predictions and actual logged usages for Pokemon $i$.
...
The distribution of usages is extremely right-skewed, with a mean of 37,946.39 far exceeding the median of 8,468, and a logarithmic-scale histogram of usages appearing vaguely symmetrical (Figure 2). Being an example of count data, it seems appropriate to model the counts as a Poisson distribution (where $\text{E}[X] = \text{Var}[X]$) or an overdispersed distribution (to account for extra variance) like the quasi-poisson or negative binomial. However, Figure 2's appearance is also vaguely normal, with very high counts and no zeroes, which may make the log-normal distribution a reasonable approximation.
...
Pokemon usage is also heavily dependent on tier and evolution: mid-to-high-level tiers like OU, UU, and RU being the most common tier by far and low-level tiers like LC and ZU being the least common; moreover, fully-evolved or non-evolving Pokemon are more popular than not fully-evolved Pokemon. Though a Pokemon's tier and evolution do not affect how it battles, the model may need to adjust for these confounders. This trend also manifests in Table 4, which lists the dataset's extremes. The top 5 most common Pokemon are all fully-evolved or non-evolving dual-types in OU with high totals, while the top 5 least common Pokemon are monotypes with low totals in LC. This trend is further visualized in Figure [A1](https://tirangol.github.io/JSC370-Project/data.html#by-tier)/[A3](https://tirangol.github.io/JSC370-Project/data.html#by-evolution-stage) in the report website, which show positive correlations between tier, evolution stage, total base statistics, and usage.
...
Figure 6 shows various 2D histograms and a box-whisker plot to assess the predictive power of shallow type-related features. It was hypothesized that Pokemon with less weaknesses, more resistances, and more super-effective coverage would be more useful competitively and thus used more often.

Apart from the number of resistances, there is little to no correlation between usage and coverage, or usage and weakness count. Perhaps poor coverage is not problematic for defensive-based Pokemon and many weaknesses is not problematic for attack-based Pokemon. It may be necessary to combine `best_stat` with model weaknesses and coverage to properly model the hypothesized effect. Finally, dual-types are more-used than monotypes, which is likely another byproduct of evolution as Pokemon commonly gain an additional type as they evolve.
...
Pokemon typing can be additionally be analyzed on a per-type level. Table 5 shows the results of three ANOVA tests performed on features for the presence of a type, defensive matchups against a type, and offensive coverage against a type, indicating to a 5% level,
...
Three linear models with different data distributions were tested -- log-normal, quasi-poisson, and negative binomial. Each model had a sample base model incorporating base statistics (totals, standard deviations, best base statistic type) and shallow type information (number of types, weaknesses, resistances, and offensive type matchups), with coefficients displayed in Figure 7. The quasi-poisson model and negative binomial model have additional dispersion parameters $\theta$ of 25154.58 (the estimated $\frac{\text{Var}[Y|X]}{\text{E}[Y|X]}$) and 1.055 (where $\frac{1}{\sqrt{\theta}} \approx 0.974$ is the relative excess standard deviation) respectively, providing strong evidence of overdispersion.

All three models generally agree on the effect of each term. Adjusting for tier and evolution status, dual-types have a higher rate of usage ($\approx 1.2$ times) than monotypes. Compared to a general attack-based Pokemon, speed-based Pokemon now have the highest usage rates than other specializations, while well-rounded Pokemon have the lowest usage rates. Usage increases with less weaknesses and more resistances, as well as with increasing standard deviation and total total base statistics, showing that specialized Pokemon are more-used competitively.

The negative binomial model is the most unusual model in terms of coefficints as it predicts the opposite effect for special bulk-based Pokemon term than the other models. It should be noted that the negative binomial model suffers from poor optimizer convergence, raising concerns over the validity of the results.
...
**Table 7**. Estimated multiplicative change to a Pokemon's rate of usage per 10-point increase in individual base statistic type, with $p$-values, after adjusting for evolution status and tier, for negative binomial GLMs using default base statistics (left) and using the bulk feature (right).
...
Based on the nonlinear positive correlations observed in Figure 3, two GAM were constructed: a "shallow model" with smooth functions for the total (+ standard deviation), and a "deep model" with smooth functions for all six base statistics (+ standard deviation). From Figure 9, total statistics appears to behave like analyzed in Figure 3, while usage consistently increases with standard deviation. For the "deep model", most base statistics follow a monotonic relationship, while special defence and speed have an optimal region of usage around 80-100 and poorly-sampled variable regions below 30 and above 125. This suggests linear regression, as previously performed, is a bad match for the latter features since the highly-variable regions are also the most influential points to a linear model. The positive correlation for speed noticed in Table 7 also disappears, which is difficult to explain.
...
Three sets of boolean features are separately added to the deep GAM model to replace some shallower type features (# of weaknesses, resistances, and coverages): the presence of, super-effective coverage of, and resistance to a type. Note that one type was excluded as it was redundant information (since Pokemon must have at least one type) that created perfect multicollinearity.

Figure 10 shows that after adjusting for evolution status, tier, number of weaknesses and resistances, and base statistics, the presence of steel, ground, poison, fire, fighting, fairy, and dark types correlate positively to usage to a significant level, while the presence of bug types contributes negatively to usage. The trend is especially prominent for steel types, which is reasonable as it is the best defensive type (see Figure 1). Resistance to bug, flying, grass, and water (i.e. bug or grass types) negatively correlates with usage, albeit barely significantly ($p \approx 0.05$). Type coverage does not appear appear to correlate with usage apart from psychic type coverage, which has a marginal  negative effect. The results suggest that type presence is the most informative type information.

\vspace{-2mm}

```{r include=FALSE}
# Presence of type
m8 <- gam(usage ~ evolution + formats + s(hp) + s(atk) + s(def) + s(spa) + s(spd) + s(spe) + typecount + s(stdev) + best_stat + is_fire + is_water + is_grass + is_electric + is_ice + is_fighting + is_poison + is_ground + is_flying + is_psychic + is_bug + is_rock + is_ghost + is_dragon + is_dark + is_steel + is_fairy, data=dataset, family=nb(link='log'), method='ML')

# Type coverage
m9 <- gam(usage ~ evolution + formats + s(hp) + s(atk) + s(def) + s(spa) + s(spd) + s(spe) + typecount + s(stdev) + best_stat + atk_fire + atk_water + atk_grass + atk_electric + atk_ice + atk_fighting + atk_poison + atk_ground + atk_flying + atk_psychic + atk_bug + atk_rock + atk_ghost + atk_dragon + atk_dark + atk_steel + atk_fairy, data=dataset, family=nb(link='log'), method='ML')

# Type defence
m10 <- gam(usage ~ evolution + formats + s(hp) + s(atk) + s(def) + s(spa) + s(spd) + s(spe) + typecount + s(stdev) + best_stat + def_fire + def_water + def_grass + def_electric + def_ice + def_fighting + def_poison + def_ground + def_flying + def_psychic + def_bug + def_rock + def_ghost + def_dragon + def_dark + def_steel + def_fairy, family=nb(link='log'), method='ML', data=dataset |>
...
A set of move-based features were added to the type-presence GAM model, with coefficients for the new features displayed in Table 8. Non-damaging and priority moves significantly contribute to greater Pokemon usage ($p < 0.05$); this seems reasonable because priority moves allow slow Pokemon to overcome their low speed, and more non-damaging moves may imply a Pokemon is versatile, with more play-styles. On the other hand, the presence of inaccurate, stat-changing, and multiturn setup moves negatively impact usage. Inaccuracy commonly balances powerful moves -- out of 115 moves with power $\geq 100$, 54 of them have accuracies $< 100%$ -- so it seems that powerful, unreliable attackers are less popular. Moves affecting base statistics are similarly risky because they sacrifice a turn for a future advantage contingent on the attacker not switching out (which removes the base statistic change). On the other hand, multiturn setup moves have effects persisting across turns, making their benefits much less risky in theory, so it is interesting to see that Pokemon using these moves are still unpopular. Furthermore, moves with heals, status effects, hazards, and protections, which are commonly seen in battles, all see non-significant impacts on usage. 
...
**Figure 11**. Feature importance plots showing the 20 most significant features of a random forest (left) and XGBoost (right) model trained on the same feature set as Table 8, after adjusting for evolution status and tier.

\vspace{2mm}

The random forest model has logged RMSEs of 0.49/1.15 on the training/testing data, while the XGBoost model has 0.41/1.26. This clear sign of overfitting is corroborated by a closer analysis of the residuals in Figure 12 revealing considerable outliers in the testing set, which may bias the feature importance plots towards continuous variables where there are more possible splits and thus ways to overfit, such as the base statistics. All models, even the final GAM model, still show diagonal line cluster artifacts, indicating there is still much unexplained variance in the data.
...
Comparing the two model types, the decision trees find all base statistics important while the GAM only finds significance for a subset, a possible result of overfitting. The GAM also finds significance in move types and typings that the decision trees seem to consider unimportant. All models find importance in the standard deviation of stats, as well as the proportion of non-damaging moves. Since many of the same conclusions are made by the decision models, the GAM model appears to be the more robust choice.
...
- In order of decreasing importance, base statistics, the presence of types, and the presence of moves all have  statistically significant effects on a Pokemon's usage across many different models.

- Usage increases by roughly 67%/20% per 10-point increase in total/standard deviation of base statistics respectively, when not controlling for specific base statistics.

- Usage correlates positively with special attack and special defence, but results are inconclusive for other base statistics. Similarly, Table 9 and Figure 7 agree that speed-based Pokemon have high rates of usage, but have conflicting results for other type specializations.
...
- The most popular type is steel, whose presence boosts usage by a factor of 3. Other types with significant positive effects are fairy (x1.9), fire (1.7), dark (1.5), and fighting (1.5). The least popular type is bug, whose presence lowers usage by a significant factor of 0.7.

- Usage increases by roughly 25% both for Pokemon with priority moves and for a 50% increase in the proportion of non-damaging moves. Usage decreases by a factor of 0.7 for Pokemon with stat-changing moves and multiturn setup moves, as well as by 0.87 for each inaccurate move it has.
